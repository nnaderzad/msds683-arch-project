...
## 8. Companion tables, join keys, and the synthetic sandbox (2026-08-08)

Tables that live alongside the diagrams above but weren't drawn — plus the new
synthetic dataset. (The text-to-SQL agent's machine-readable view of all of this
is generated into `api/schema_context.md` by `eda/build_schema_context.py`.)

### Join-key normalization (moved here from local-only notes — load-bearing)

- **`artist_id` is a deterministic hash of the normalized artist name**
  (`common/keys.py::artist_id`). Ticketmaster and Google Trends must emit
  **identical normalized names** or the join silently drops rows. In rosters,
  `query` (the term sent to Trends) may differ from `name` (display) to
  disambiguate common words (`Fisher` → `"Fisher DJ"`).
- **`dma_code` is the bare Nielsen code** (`807`), never the Trends geo string
  (`US-CA-807`); `common/keys.py::bare_dma()` strips it for joins.
- `venue_id` is a hash of the TM venue id (`common/keys.py::venue_id`).

### Silver companions (queryable, not in the ER diagrams)

- **`tm_observations`** — the raw observed-only price history the cloud function
  appends (`event_id, snapshot_date, status_code, price_*, sale windows,
  first/last_seen_ts_utc`). `fact_ticketmaster` supersedes it for analysis (same
  grain + `n_captures`/`price_disagreed` provenance).
- **`tm_events`** — current-state MERGE of every event ever seen (last price
  carried forward). Feeds the dims build. **Never source price history here.**
- **`fact_trends_daily`** — `(artist_id, dma_code, snapshot_date=interest day,
  interest, granularity, is_partial)`. The iot **daily trajectory** (deep,
  backfillable); its 0–100 is comparable across *time* within one (artist, DMA) —
  vs `fact_trends`' cross-metro snapshot normalization. Gold's `local_interest`
  sources from here first, ibr fallback.
- **Scene facts** (2026-07-08, Bay-Area scene sources):
  - `fact_nineteenhz` — 19hz.info listing per `(event, snapshot_date)`: venue,
    date, price text (74.6% priced), lineup, event URL, organizers, age policy.
  - `fact_ra` — Resident Advisor per `(event, snapshot_date)` incl. `attending`
    (re-observed daily = a buzz series). Strictly 1 automated request/day.
  - `fact_ticketpages` — JSON-LD offers per `(offer, page, snapshot_date)` from
    eventbrite/shotgun: price, currency, `availability` (incl. SoldOut).

### Gold companion

- **`forecast_event_price`** — `(event_id, days_to_show, predicted_price)`, the
  anchor+drift model's precomputed curve from run day to show day.
  ⚠️ **No date column** — freshness is only observable via the table's
  `lastModifiedTime` (this is how a 39-day-stale forecast hid in July; the
  freshness SQL in REPO_STATE can't see this table).

### `event_demand_synth` — the synthetic sandbox (separate dataset)

Hybrid design: **real event spine** (real events/artists/venues, researched
capacities from `reference/venue_capacities.csv`, observed face prices) with
**synthetic infill only where no source exists** — sellout timing and the resale
market (TM bronze has a measured **0.0%** resale priceRanges). Generated by
`synth/generate.py` from the seeded heuristics in `synth/heuristics.py`
(same seed → identical world).

- `synth_event_demand` — one row per real event: `demand_ratio`,
  `sellout_probability`, `sold_out`, `sellout_date`, `resale_multiplier`,
  `resale_price_at_show`, plus labels: `capacity_source`
  (`researched`/`tier_estimate`), `face_price_source`
  (`observed`/`genre_median`/`type_default`), and provenance
  (`synth_run_id`, `generator_version`, `seed`).
- `synth_resale_series` — resale price trajectory per event over
  `days_to_show` checkpoints.

Rules: synthetic rows **never** enter `event_demand_analytics`; the text-to-SQL
agent reaches this dataset only via the explicit `dataset: "synth"` toggle with
a **disjoint table allow-list**, and every answer is labeled synthetic.
