...
- **Search** — `GET /search` (genre / artist / geo / max price / horizon) over the
  gold star; `GET /genres` feeds the dropdown.
...
flowchart TD
    ...
    P --> AST["sqlglot AST — single SELECT only, no DML/DDL nodes,<br/>table allow-list (12 tables; traps excluded)"]
...
`INFORMATION_SCHEMA` + curated semantic notes + few-shot examples that are
dry-run-validated at generation time. Quality is measured, not vibed:
`eda/eval_text_to_sql.py` scores 26 questions × 3 runs by **execution-result
match** against gold SQL — currently **92%** overall (easy 100% · join 86% ·
aggregate 83% · trick/refusal 100%), with a failure taxonomy in
[`eda/output/text_to_sql_eval.md`](../eda/output/text_to_sql_eval.md).
...
| **Partitioning + clustering** (50× scale twins, 54.8 M rows / 3.98 GiB each) | flat table: single-day scan 417.9 MiB; event history 1,501 MiB | partitioned+clustered: **16.2 MiB (−96%)**; **310 MiB (−79%)** — cluster pruning only visible in *actual* bytes, dry-run can't see it |

Cost: storage pennies/month, BigQuery within free tier, agent ≈ $0.003/question —
details in the root README's cost section.
