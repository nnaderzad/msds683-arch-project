...
natural-language answer. Every layer emits a verdict the UI can badge:
  ...
  4. Execution caps — ``maximum_bytes_billed`` 1 GiB, ``LIMIT`` injection,
     20 s timeout, response truncated to 50 rows.
...
def build_sql_prompt(schema_context: str, question: str, previous_error: str | None = None) -> str:
    repair = ""
    if previous_error:
        repair = (
...
class Text2SqlService:
    ...
    def ask(self, question: str) -> dict[str, Any]:
        ...
        sql, refusal, error = self._generate_sql(question)
        ...
        sql, dry_bytes, dry_verdict = self._dry_run_with_repair(question, sql)
    ...
    def _generate_sql(
        self, question: str, previous_error: str | None = None
    ) -> tuple[str | None, str | None, str | None]:
        """Return (sql, refusal_reason, error) — exactly one is non-None."""
        try:
            raw = self.llm.generate(
                build_sql_prompt(self.schema_context, question, previous_error)
            )
    ...
    def _dry_run_with_repair(
        ...
        for attempt in (0, 1):
            try:
                ...
                repaired, refusal, gen_error = self._generate_sql(question, str(err))
                if repaired is None:
