...
def load_presence_series(project: str, dataset: str) -> list[dict[str, str]]:
    ...
    sql = f"""
        ...
        FROM `{project}.{dataset}.{FACT_TABLE}` AS f
...
def write_outputs(out_dir: Path, classified: list[dict], summary: dict, as_of: str) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    with (out_dir / "tm_price_gap_diagnosis_by_event.csv").open(
    ...
    lines = [
        ...
        "",
        f"_Generated by `eda/diagnose_price_gaps.py`, as of {as_of}, over "
    ...
    (out_dir / "tm_price_gap_diagnosis.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    as_of = utc_now_iso()
    rows = load_presence_series(args.project, args.dataset)
    if not rows:
        print("[diagnose_price_gaps] no rows in fact_ticketmaster", file=sys.stderr)
        return 1
    dates = window_dates(rows)
    classified = classify_event_gaps(rows, dates)
    summary = summarize_gaps(classified, len(dates))

    write_outputs(Path(args.output_dir), classified, summary, as_of)
